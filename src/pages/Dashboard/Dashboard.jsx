...
import {getPasswordHash} from "../../utils/cookies.js";
...
export default function Dashboard() {
    ...
    const handleEnteredPassword = useCallback(async () => {
        ...
        const match = await comparePasswordWithHash(password, getPasswordHash())
        if (match) {
            setIsOnError(false);
    ...
    }, [hasNoteChanged, getNoteByID, getLatestNoteVersionByNoteID])
    ...
    // Handle the note update modal
    const handleNoteUpdateModal = useCallback(() => {
        setIsUpdateNoteModalOpen((prevState) => !prevState)
    }, [])
    ...
    const handleNoteEdit = useCallback((id) => {
        if (hasNoteChanged) {
            setIsUpdateNoteModalOpen(true)
            return
        }
        ...
        const noteContent = id ? getLatestNoteVersionByNoteID(id)?.content : null

        setIsNoteContainerOpen(true)
    ...
    }, [removeNoteByID, selectedNote, handleNoteDeletionModal])

    // Handle the note update
    const handleNoteUpdate = useCallback(() => {
        // Get latest note version ID
        const latestNoteVersionID = getLatestNoteVersionID(selectedNote.id)

        upsertNoteVersion({
            id: latestNoteVersionID? latestNoteVersionID + 1 : 1,
            note_id: selectedNote.id,
            content: editNoteContent,
            create_at: new Date(),
            updated_at: new Date()
        })
        handleNoteUpdateModal()
    }, [getLatestNoteVersionID, selectedNote, upsertNoteVersion, editNoteContent, handleNoteUpdateModal])
    ...
    const handleNoteBlur = useCallback((event) => {
        ...
        if (selectedNoteContent !== editNoteContent) setHasNoteChanged(true)
    }, [editNoteContent, selectedNoteContent])
    ...
    return (
        <>
            ...
            {isCreateNoteModalOpen && (
                <Modal header={(
                    <TitleText>New Note</TitleText>
                       ...
                       style={newNoteColor && {borderColor: newNoteColor}}>
                    <CallToAction
                ...
                <Modal header={(
                    ...
                    <div
                        ...
                        <SecondaryButton className='button--secondary--unfilled'
                                         onClick={handleNoteDeletion}>Yes</SecondaryButton>
                        <SecondaryButton
            ...
            {isUpdateNoteModalOpen && (
                <Modal header={(
                    <TitleText>Update Note</TitleText>
                )} className='dashboard__note-update-modal'
                       onClose={handleNoteUpdateModal}
                       style={{borderColor: selectedNote.color}}>
                    <ParagraphText>Are you sure you want to update this
                        note?</ParagraphText>
                    <div
                        className='dashboard__note-update-modal__buttons-container'>
                        <SecondaryButton className='button--secondary--unfilled'
                                         onClick={handleNoteUpdateModal}>No</SecondaryButton>
                        <SecondaryButton
                            onClick={handleNoteUpdate}>Yes</SecondaryButton>
                    </div>
                </Modal>
            )}
            <div className='dashboard__main-container'>
                ...
                <div
                    ...
                    <div key={selectedNote?.id}
                         className='dashboard__main-container__note-container__content-container'>
